...
def superflux(y, fs, window_length=51, polyorder=3):
    ...
    envelope_sf = smooth(envelope_sf, window_length, polyorder) 
...
def default_onset(y, fs, window_length=51, polyorder=3):
    ...
    env_default = smooth(env_default, window_length, polyorder) # window size 51, polynomial order 3
...
def overlay(col_dir, y_dir, position_seed=0, position_y=0):
	...
	if position > 0:
		rec = collective.overlay(y, position=position)
	else:
...
def align(rec_dir, collective_dir, seed_onset):
	...
	y_onset = superflux(y_harmonic, fs)
